...
static const u32 cdReadTime = ((PSXCLK / 75) / BIAS);	// 0x37200
...
static void StartReading(u32 type) {
   	cdr.Reading = type;
  	cdr.FirstSector = 1;
  	cdr.Readed = 0xff;
...
static void SetResultSize(u8 size) {
    cdr.ResultP = 0;
	cdr.ResultC = size;
	cdr.ResultReady = 1;
}
...
#endif
			SysPrintf("cdrWrite1() Log: Unknown command: %x\n", cdr.Cmd);
			return;
    }
	if (cdr.Stat != NoIntr) {
		psxRaiseExtInt( PsxInt_CDROM );
