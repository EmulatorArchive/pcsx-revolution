...
void psxBios_SetMem() { // 9f
	...
	switch(a0) {
		case 2:
			psxHu32ref(0x1060) = SWAP32(cnew);
			psxMu32ref(0x060) = a0;
			SysPrintf("Change effective memory : %d MBytes\n",a0);
			break;

		case 8:
			psxHu32ref(0x1060) = SWAP32(cnew | 0x300);
			psxMu32ref(0x060) = a0;
			SysPrintf("Change effective memory : %d MBytes\n",a0);
...
void psxBios_SetRCnt() { // 02
	...
	if (a0 != 3) {
		...
		psxRcntWtarget((PsxEventType)a0, a1);
		...
		psxRcntWmode((PsxEventType)a0, mode);
	}
	pc0 = ra;
}
...
void psxBios_GetRCnt() { // 03
	...
	a0&= 0x3;
	if (a0 != 3) v0 = psxRcntRcount((PsxEventType)a0);
	else v0 = 0;
	pc0 = ra;
}
...
void psxBios_ResetRCnt() { // 06
	...
	a0&= 0x3;
	if (a0 != 3) {
		psxRcntWmode((PsxEventType)a0, 0);
		psxRcntWtarget((PsxEventType)a0, 0);
		psxRcntWcount((PsxEventType)a0, 0);
	}
	pc0 = ra;
}
