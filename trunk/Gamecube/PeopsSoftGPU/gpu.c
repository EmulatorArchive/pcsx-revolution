...
#ifdef __GX__
void CALLBACK PEOPS_GPUdisplayText(char * pText)             // some debug func
#else
...
////////////////////////////////////////////////////////////////////////
#ifdef __GX__
long CALLBACK PEOPS_GPUinit()                                // GPU INIT
#else
...
#ifdef __GX__
long CALLBACK PEOPS_GPUclose()                               // GPU CLOSE
#else
...
#ifdef __GX__
long CALLBACK PEOPS_GPUshutdown()                            // GPU SHUTDOWN
#else
...
void updateDisplay(void)                               // UPDATE DISPLAY
{
	...
	if(UseFrameSkip)                                      // skip ?
	{
		...
		if(bSkipNextFrame) {
			++consec_skipped;
			if( consec_skipped > 1 )  // Config.FramesToSkip
			{
				consec_skipped = 0;
		...
		{
			++consec_drawn;
			if( consec_drawn > 6 ) // Config.FramesToDraw
			{
				consec_drawn = 0;
				bSkipNextFrame = TRUE;
			}
...
#ifdef __GX__
void CALLBACK PEOPS_GPUupdateLace(void)                      // VSYNC
#else
...
#ifdef __GX__
uint32_t CALLBACK PEOPS_GPUreadStatus(void)             // READ STATUS
#else
...
////////////////////////////////////////////////////////////////////////
#ifdef __GX__
void CALLBACK PEOPS_GPUwriteStatus(uint32_t gdata)      // WRITE STATUS
#else
...
#ifdef __GX__
void CALLBACK PEOPS_GPUreadDataMem(uint32_t * pMem, int iSize)
#else
...
#ifdef __GX__
uint32_t CALLBACK PEOPS_GPUreadData(void)
#else
...
#ifdef __GX__
void CALLBACK PEOPS_GPUwriteDataMem(uint32_t * pMem, int iSize)
#else
...
#ifdef __GX__
void CALLBACK PEOPS_GPUwriteData(uint32_t gdata)
#else
...
#ifdef __GX__
long CALLBACK PEOPS_GPUfreeze(uint32_t ulGetFreezeData,GPUFreeze_t * pF)
#else
